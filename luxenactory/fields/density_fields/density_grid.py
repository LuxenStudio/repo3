...
class DensityGrid(nn.Module):
    ...
    def __init__(
        ...
        grid_indices = luxenactory_cuda.morton3D(grid_coords.to("cuda:0")).to("cpu")
    ...
    def sample_uniform_and_occupied_cells(self, n: int) -> List[Tuple[TensorType["n"], TensorType["n", 3]]]:
        ...
        for c in range(self.num_cascades):
            ...
            uniform_indices = luxenactory_cuda.morton3D(uniform_coords)
            ...
            occupied_coords = luxenactory_cuda.morton3D_invert(occupied_indices)
    ...
    def update_density_grid(
        ...
        # pack to bitfield
        self.density_bitfield.data = luxenactory_cuda.packbits(
            self.density_grid, min(self.mean_density.item(), density_threshold)
        )

        # TODO(ruilongli): max pooling? https://github.com/NVlabs/instant-ngp/blob/master/src/testbed_luxen.cu#L578
    ...
    def get_densities(self, ray_samples: RaySamples) -> TensorType[..., 1]:
        ...
        _mip_levels, indices, _occupancies = luxenactory_cuda.occupancy_query(
