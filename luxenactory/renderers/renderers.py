...
class RGBRenderer(nn.Module):
    ...
    def combine_rgb(
        ...
        if ray_indices is not None and num_rays is not None:
            rgb = luxenacc.volumetric_rendering_accumulate(weights, ray_indices, rgb, num_rays)
            accumulated_weight = luxenacc.volumetric_rendering_accumulate(weights, ray_indices, None, num_rays)
        else:
...
class AccumulationRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            accumulation = luxenacc.volumetric_rendering_accumulate(weights, ray_indices, None, num_rays)
        else:
...
class DepthRenderer(nn.Module):
    ...
    def forward(
        ...
        if self.method == "expected":
            ...
            if ray_indices is not None and num_rays is not None:
                depth = luxenacc.volumetric_rendering_accumulate(weights, ray_indices, steps, num_rays)
            else:
