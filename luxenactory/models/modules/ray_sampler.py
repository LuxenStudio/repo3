...
from luxenactory.cameras.rays import Frustums, RayBundle, RaySamples
...
class NGPSpacedSampler(Sampler):
    ...
    def forward(
        ...
        Args:
            ...
            num_samples: Number of samples per ray
            cone_angle: 0.0 for luxen-synthetic and 1.0/256 for real scenes
        ...
        t_min, t_max = luxenactory_cuda.ray_aabb_intersect(rays_o, rays_d, aabb)
        ...
        # marching
        packed_info, origins, dirs, starts, ends = luxenactory_cuda.raymarching(
            # rays
            rays_o,
            rays_d,
            t_min,
        ...
        indices = luxenactory_cuda.get_camera_indices(packed_info, ray_bundle.camera_indices, indices)
...
class VolumetricSampler(Sampler):
    ...
    def __init__(
        ...
        self.occ_field = luxenacc.OccupancyField(occ_eval_fn=occ_eval_fn, aabb=aabb, resolution=grid_resolution)
    ...
    def forward(
        ...
        packed_info, starts, ends = luxenacc.volumetric_marching(
            rays_o=rays_o,
            rays_d=rays_d,
            aabb=self.aabb,
        ...
        ray_indices = luxenacc.unpack_to_ray_indices(packed_info).long()
        ...
        if remove_occluded_samples:
            with torch.no_grad():
                ...
                if camera_indices is not None:
                    packed_info, starts, ends, origins, dirs, camera_indices = luxenacc.volumetric_rendering_steps(
                        packed_info, densities, starts, ends, origins, dirs, camera_indices
                    )
                else:
                    packed_info, starts, ends, origins, dirs = luxenacc.volumetric_rendering_steps(
