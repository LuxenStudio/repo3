...
from luxenactory.dataloaders.base import Dataloader, VanillaDataloader
from luxenactory.dataloaders.datasets import Blender, Dataset, Friends, Mipluxen360
...
class TrainerConfig(PrintableConfig):
    ...
    relative_model_dir: Path = Path("luxenactory_models/")
...
@dataclass
class MipLuxen360DatasetConfig(InstantiateConfig):
...
@dataclass
class MipLuxen360DataloaderConfig(DataloaderConfig):
    """Mipluxen 360 dataloader config"""

    _target: Type = VanillaDataloader
    train_dataset: InstantiateConfig = MipLuxen360DatasetConfig()
...
@dataclass
class LuxenWModelConfig(ModelConfig):
    """LuxenW model config"""

    _target: Type = LuxenWModel
