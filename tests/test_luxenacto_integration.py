...
def run_ns_download_data(scene: Literal["poster", "dozer", "desolation"]):
    command = f"ns-download-data luxenstudio --capture-name={scene}"
    run_command_with_console_output(command)


def run_ns_train_luxenacto(scene: Literal["poster", "dozer", "desolation"]):
    dataset_path = f"data/luxenstudio/{scene}"
    command = f"ns-train luxenacto --data {dataset_path}"
...
def run_ns_eval(scene: Literal["poster", "dozer", "desolation"]):
    timestamp = sorted(os.listdir(f"outputs/{scene}/luxenacto/"))[-1]
    config_filename = f"outputs/{scene}/luxenacto/{timestamp}/config.yml"
    command = f"ns-eval --load-config {config_filename} --output-path luxenacto_integration_eval.json"
    run_command_with_console_output(command)

    with open("luxenacto_integration_eval.json", "r") as f:
        results = json.load(f)
