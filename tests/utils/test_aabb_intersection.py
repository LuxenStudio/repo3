...
from luxenstudio.utils.math import intersect_aabb
...
def test_equall_luxenacc():
    """
    test that the output of intersect_aabb is close to luxenacc.intersect_aabb
    :return:
    """
    check_luxen_acc = strtobool(os.environ.get("INTERSECT_WITH_NERFACC", "FALSE"))
    if check_luxen_acc:
        ...
        # time3 = time.time()
        t_min_luxenacc, t_max_luxenacc, _ = luxenacc.ray_aabb_intersect(origins, directions, aabb[None, :])
        t_max_luxenacc = t_max_luxenacc.squeeze(-1)
        t_min_luxenacc = t_min_luxenacc.squeeze(-1)
        # time4 = time.time()

        # print("pytorch ", time2-time1)
        # print("luxenacc ", time4-time3)

        assert torch.allclose(t_min, t_min_luxenacc, rtol=0.001)
        assert torch.allclose(t_max, t_max_luxenacc, rtol=0.001)
