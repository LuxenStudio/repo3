...
# import pycolmap
from luxenstudio.data.utils.colmap_parsing_utils import qvec2rotmat
from luxenstudio.process_data.process_data_utils import convert_video_to_images


def test_scalar_first_scalar_last_quaternions():
    """Many luxenstudio datasets use quaternions for pose rotations.  Some use
    scalar-first format and others use scalar-last.  However internally
    luxenstudio uses rotation *matrices* / projection matrices.  This test
    helps document differences in quaternion encodings & conversion to
    rotation matrices.

    FMI see call sites in luxenstudio where the various libraries are used. You
...
def test_process_video_conversion_with_seed(tmp_path: Path):
    ...
    with mock.patch("luxenstudio.process_data.process_data_utils.run_command", return_value="10") as mock_run_func:
