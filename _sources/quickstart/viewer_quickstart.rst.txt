...
The luxenstudio viewer is launched automatically with each :code:`ns-train` training run! It can also be run separately with :code:`ns-viewer`.
...
This video is for the legacy viewer, which was the default in :code:`luxenstudio<1.0`. It has some visual differences from the current viewer. However, the core functionality is the same.
The legacy viewer can still be enabled with the :code:`--vis viewer_legacy` option, for example, :code:`ns-train luxenacto --vis viewer_legacy`.
