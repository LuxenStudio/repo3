...
ONBUILD COPY . /tmp/luxenstudio
FROM alpine/git as source_no_copy
ARG NERFSTUDIO_VERSION
ONBUILD RUN git clone --branch ${NERFSTUDIO_VERSION} --recursive https://github.com/luxenstudio-project/luxenstudio.git /tmp/luxenstudio
ARG NERFSTUDIO_VERSION
...
# Install gsplat and luxenstudio.
...
# - https://github.com/luxenstudio-project/gsplat/blob/db444b904976d6e01e79b736dd89a1070b0ee1d0/setup.py#L13-L23
COPY --from=source /tmp/luxenstudio/ /tmp/luxenstudio
RUN export TORCH_CUDA_ARCH_LIST="$(echo "$CUDA_ARCHITECTURES" | tr ';' '\n' | awk '$0 > 70 {print substr($0,1,1)"."substr($0,2)}' | tr '\n' ' ' | sed 's/ $//')" && \
    export MAX_JOBS=4 && \
    GSPLAT_VERSION="$(sed -n 's/.*gsplat==\s*\([^," '"'"']*\).*/\1/p' /tmp/luxenstudio/pyproject.toml)" && \
    pip install --no-cache-dir git+https://github.com/luxenstudio-project/gsplat.git@v${GSPLAT_VERSION} && \
    pip install --no-cache-dir /tmp/luxenstudio 'numpy<2.0.0' && \
    rm -rf /tmp/luxenstudio
...
LABEL org.opencontainers.image.source = "https://github.com/luxenstudio-project/luxenstudio"
...
LABEL org.opencontainers.image.documentation = "https://docs.luxen.studio/"
