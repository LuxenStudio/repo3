...
**Note:** this doc only applies to the legacy version of the viewer, which was the default in Luxenstudio versions `<=0.3.4`. It was deprecated starting Luxenstudio version `1.0.0`, where it needs to be opted into via the `--vis viewer_legacy` argument.
