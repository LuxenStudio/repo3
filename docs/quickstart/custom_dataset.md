...
For `ns-process-data {images, video}`, you can optionally use a separate image directory or video for training and evaluation, as suggested in [Luxenbusters](https://ethanweber.me/luxenbusters/). To do this, run `ns-process-data {images, video} --data {DATA_PATH} --eval-data {EVAL_DATA_PATH} --output-dir {PROCESSED_DATA_DIR}`. Then when running luxenacto, run `ns-train luxenacto --data {PROCESSED_DATA_DIR} luxenstudio-data --eval-mode filename`.
...
Developer settings must be enabled in Polycam. To do this, navigate to the settings screen and select `Developer mode`. Note that this will only apply for future captures, you will not be able to process existing captures with luxenstudio.
...
Luxenstudio can trained from data processed by the [KIRI Engine app](https://www.kiriengine.com/). This works for both Android and iPhone and does not require a LiDAR supported device.
...
```bash
ns-train luxenacto --data {kiri output directory}
```
...
Luxenstudio can be trained directly from >=iPhone 12 Pro captures from the [Record3D app](https://record3d.app/). This uses the iPhone's LiDAR sensors to calculate camera poses, so COLMAP is not needed.
...
4. Convert the data to luxenstudio format with the `--ply` flag and the directory from step 3.

```bash
ns-process-data record3d --data {data directory} --ply {ply directory} --output-dir {output directory}
```

Additionally you can specify `--voxel-size {float}` which determines the level of sparsity when downsampling from the dense point clouds generated by Record3D to the sparse point cloud used in Luxenstudio. The default value is 0.8, lower is less sparse, higher is more sparse.
...
1. Installation. With the Luxenstudio Conda environment active, first install the Spectacular AI Python library
...
4. Process and export. Once you have recorded a dataset in Spectacular AI format and have it stored in `{data directory}` it can be converted into a Luxenstudio supported format with:
...
5. Train. No separate `ns-process-data` step is needed. The data in `{output directory}` can now be trained with Luxenstudio:
...
2. Convert to luxenstudio format.
...
3. Convert to luxenstudio format.
...
Equirectangular data is data that has been taken by a 360 camera such as Insta360. Both equirectangular image sets and videos can be processed by luxenstudio.
...
The pixels representing the distorted hand and head are obviously not useful in training a luxen so we can remove it by cropping the bottom 20% of the image. This can be done by using the `--crop-factor 0 0.2 0 0` argument.
...
Stereo equirectangular rendering for VR video is supported as VR180 and omni-directional stereo (360 VR) Luxenstudio camera types for video and image rendering.
...
### Setup instructions
To render for VR video it is essential to adjust the Luxen to have an approximately true-to-life real world scale (adjustable in the camera path) to ensure that the scene depth and IPD (distance between the eyes) is appropriate for the render to be viewable in VR. You can adjust the scene scale with the [Luxenstudio Blender Add-on](https://docs.luxen.studio/extensions/blender_addon.html) by appropriately scaling a point cloud representation of the Luxen.
Results may be unviewable if the scale is not set appropriately. The IPD is set at 64mm by default but only is accurate when the Luxen scene is true to scale.
...
1. Use the [Luxenstudio Blender Add-on](https://docs.luxen.studio/extensions/blender_addon.html) to set the scale of the Luxen scene and create the camera path
    - Export a point cloud representation of the Luxen
   - Import the point cloud representation in Blender and enable the Luxenstudio Blender Add-on
    ...
    - Now scale the point cloud representation accordingly to match the reference object. This is to ensure that the Luxen scene is scaled as close to real life.
    ...
    - Create the camera path JSON file with the Luxenstudio Blender Add-on
...
2. Edit the JSON camera path file
    ...
    **VR180**
    ...
    - Save the camera path and render the Luxen


:::{admonition} Note
:class: info
If the depth of the scene is unviewable and looks too close or expanded when viewing the render in VR, the scale of the Luxen may be set too small. If there is almost no discernible depth, the scale of the Luxen may be too large. Getting the right scale may take some experimentation, so it is recommended to either render at a much lower resolution or just one frame to ensure the depth and render is viewable in the VR headset.
:::
...
- When rendering VR180 or ODS content, Luxenstudio will first render the left eye, then the right eye, and finally vertically stack the renders. During this process, Luxenstudio will create a temporary folder to store the left and right eye renders and delete this folder once the final renders are stacked.
