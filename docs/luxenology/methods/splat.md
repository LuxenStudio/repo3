...
### Running the Method
To run gaussian splatting, run `ns-train gaussian-splatting --data <data>`. Just like Luxen methods, the splat can be interactively viewed in the web-viewer, loaded from a checkpoint, rendered, and exported.
...
Luxenstudio gaussian splat exports currently supports multiple third-party splat viewers:
