    ...
    def _load_checkpoint(self) -> None:
        ...
        if load_dir is not None:
            ...
            if load_step is None:
                print("Loading latest Luxenstudio checkpoint from load_dir...")
            ...
            CONSOLE.print(f"Done loading Luxenstudio checkpoint from {load_path}")
            ...
            if "schedulers" in loaded_state and self.config.load_scheduler:
                self.optimizers.load_schedulers(loaded_state["schedulers"])
            self.grad_scaler.load_state_dict(loaded_state["scalers"])
            CONSOLE.print(f"Done loading Luxenstudio checkpoint from {load_checkpoint}")
        else:
            CONSOLE.print("No Luxenstudio checkpoint to load, so training from scratch.")
