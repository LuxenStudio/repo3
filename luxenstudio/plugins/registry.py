...
from luxenstudio.data.dataparsers.base_dataparser import DataParserConfig
from luxenstudio.engine.trainer import TrainerConfig
from luxenstudio.plugins.types import DataParserSpecification, MethodSpecification
...
def discover_methods() -> t.Tuple[t.Dict[str, TrainerConfig], t.Dict[str, str]]:
    ...
    methods = {}
    descriptions = {}
    discovered_entry_points = entry_points(group="luxenstudio.method_configs")
...
def discover_dataparsers() -> t.Tuple[t.Dict[str, DataParserConfig], t.Dict[str, str]]:
    """
    Discovers all DataParsers registered using the `luxenstudio.dataparser_configs` entrypoint.
    And also DataParsers in the NERFSTUDIO_DATAPARSER_CONFIGS environment variable.
    """
    configs = {}
    descriptions = {}
    discovered_entry_points = entry_points(group="luxenstudio.dataparser_configs")
    for name in discovered_entry_points.names:
        spec = discovered_entry_points[name].load()
