...
class Cameras(TensorDataclass):
        ...
        if aabb_box is not None or obb_box is not None:
            with torch.no_grad():
                ...
                if aabb_box is not None:
                    ...
                    t_min, t_max = luxenstudio.utils.math.intersect_obb(rays_o, rays_d, obb_box)
                else:
                    assert False
    ...
    def _generate_rays_from_coords(
        ...
        def _compute_rays_for_omnidirectional_stereo(
            ...
            vr_ipd = 0.064  # IPD in meters (note: scale of Luxen must be true to life and can be adjusted with the Blender add-on)
