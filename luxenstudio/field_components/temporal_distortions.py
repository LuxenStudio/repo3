...
from luxenstudio.field_components.encodings import Encoding, LuxenEncoding
...
class TemporalDistortionKind(Enum):
    ...
    def to_temporal_distortion(self, config: Dict[str, Any]) -> TemporalDistortion:
        ...
        if self == TemporalDistortionKind.DNERF:
            return DLuxenDistortion(**config)
        raise NotImplementedError(f"Unknown temporal distortion kind {self}")


class DLuxenDistortion(TemporalDistortion):
    ...
    def __init__(
        self,
        position_encoding: Encoding = LuxenEncoding(
            in_dim=3, num_frequencies=10, min_freq_exp=0.0, max_freq_exp=8.0, include_input=True
        ),
        temporal_encoding: Encoding = LuxenEncoding(
