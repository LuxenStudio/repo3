...
class RayBatchStream(IterableDataset):
    ...
    def __init__(
        ...
        num_times_to_repeat_images: Union[int, float] = float("inf"),
        device: Union[torch.device, str] = "cpu",
        # variable_res_collate avoids np.stack'ing images, which allows it to be much faster than `luxenstudio_collate`
    ...
    def _get_collated_batch(self, indices):
        """Takes the output of _get_batch_list and collates them with luxenstudio_collate() or variable_res_collate()
