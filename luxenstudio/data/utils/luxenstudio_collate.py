...
"""
Custom collate function that includes cases for luxenstudio types.
"""
...
def luxenstudio_collate(batch: Any, extra_mappings: Union[Dict[type, Callable], None] = None) -> Any:
    r"""
    ...
    pytorch version '1.12.1+cu113'. Custom luxenstudio types are accounted for at the end, and extra
    ...
    from default_collate to luxenstudio_collate, and the addition of the luxenstudio_collate_err_msg_format variable.
    ...
    Here is the general input type (based on the type of the element within the batch) to output type mapping:
        ...
        * `Mapping[K, V_i]` -> `Mapping[K, luxenstudio_collate([V_1, V_2, ...])]`
        * `NamedTuple[V1_i, V2_i, ...]` -> `NamedTuple[luxenstudio_collate([V1_1, V1_2, ...]),
          luxenstudio_collate([V2_1, V2_2, ...]), ...]`
        * `Sequence[V1_i, V2_i, ...]` -> `Sequence[luxenstudio_collate([V1_1, V1_2, ...]),
          luxenstudio_collate([V2_1, V2_2, ...]), ...]`
    ...
    Examples:
        ...
        >>> luxenstudio_collate([0, 1, 2, 3])
        tensor([0, 1, 2, 3])
        >>> # Example with a batch of `str`s:
        >>> luxenstudio_collate(['a', 'b', 'c'])
        ['a', 'b', 'c']
        >>> # Example with `Map` inside the batch:
        >>> luxenstudio_collate([{'A': 0, 'B': 1}, {'A': 100, 'B': 100}])
        {'A': tensor([  0, 100]), 'B': tensor([  1, 100])}
        >>> # Example with `NamedTuple` inside the batch:
        >>> Point = namedtuple('Point', ['x', 'y'])
        >>> luxenstudio_collate([Point(0, 0), Point(1, 1)])
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ in ("ndarray", "memmap"):
            ...
            return luxenstudio_collate([torch.as_tensor(b) for b in batch], extra_mappings=extra_mappings)
        ...
        try:
            return elem_type(
                {key: luxenstudio_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
            )
        except TypeError:
            # The mapping type may not support `__init__(iterable)`.
            return {key: luxenstudio_collate([d[key] for d in batch], extra_mappings=extra_mappings) for key in elem}
    elif isinstance(elem, tuple) and hasattr(elem, "_fields"):  # namedtuple
        return elem_type(*(luxenstudio_collate(samples, extra_mappings=extra_mappings) for samples in zip(*batch)))
        ...
        if isinstance(elem, tuple):
            return [
                luxenstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed
            ]  # Backwards compatibility.
        else:
            try:
                return elem_type([luxenstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed])
            except TypeError:
                # The sequence type may not support `__init__(iterable)` (e.g., `range`).
                return [luxenstudio_collate(samples, extra_mappings=extra_mappings) for samples in transposed]

    # LuxenStudio types supported below
