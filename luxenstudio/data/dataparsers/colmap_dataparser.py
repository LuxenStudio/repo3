...
from luxenstudio.cameras.cameras import CAMERA_MODEL_TO_TYPE, Cameras
...
from luxenstudio.data.utils import colmap_parsing_utils as colmap_utils
from luxenstudio.data.utils.dataparsers_utils import (
    get_train_eval_split_all,
    get_train_eval_split_filename,
    get_train_eval_split_fraction,
    get_train_eval_split_interval,
)
from luxenstudio.process_data.colmap_utils import parse_colmap_camera_params
from luxenstudio.utils.rich_utils import CONSOLE, status
...
class ColmapDataParserConfig(DataParserConfig):
    ...
    while luxenstudio world set z direction to be up direction for viewer. Therefore, we usually need to apply an extra
    ...
    Interval uses every nth frame for eval (used by most academic papers, e.g. MipLuxen360, GSplat).
...
class ColmapDataParser(DataParser):
    ...
    can be used as a drop-in replacement. It further supports datasets like Mip-Luxen 360 (although
    in the case of Mip-Luxen 360 the downsampled images may have a different resolution because they
    ...
    def _get_all_images_and_cameras(self, recon_dir: Path):
        ...
        for im_id in ordered_im_id:
            ...
            if self.config.assume_colmap_world_coordinate_convention:
                # world coordinate transform: map colmap gravity guess (-y) to luxenstudio convention (+z)
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs):
        ...
        for frame in meta["frames"]:
            ...
            if any([k in frame and float(frame[k]) != 0.0 for k in ["k4", "k5", "k6"]]):
                raise ValueError(
                    "K4/K5/K6 is non-zero! Note that Luxenstudio camera model's K4 has different meaning than colmap "
                    "OPENCV camera model K4. Luxenstudio's K4 is the 4-th order of radial distortion coefficient, while "
