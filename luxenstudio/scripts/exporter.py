...
from luxenstudio.data.datamanagers.full_images_datamanager import FullImageDatamanager
from luxenstudio.data.datamanagers.parallel_datamanager import ParallelDataManager
...
from luxenstudio.utils.spherical_harmonics import rotate_spherical_harmonics
    ...
    if normal_method == "model_output":
        ...
        if normal_output_name not in outputs:
            ...
            CONSOLE.print(
                ...
                "(e.g., luxenacto with predicted normals turned on)."
            )
...
class ExportPointCloud(Exporter):
    ...
    scaled and reoriented coordinate space expected by the Luxen models."""
...
class ExportTSDFMesh(Exporter):
    ...
    """Minimum of the bounding box, used if use_bounding_box is True."""
    texture_method: Literal["tsdf", "luxen"] = "luxen"
    """Method to texture the mesh with. Either 'tsdf' or 'luxen'."""
    ...
    def main(self) -> None:
        ...
        # possibly
        # texture the mesh with Luxen and export to a mesh.obj file
        # and a material and texture file
        if self.texture_method == "luxen":
            # load the mesh from the tsdf export
            mesh = get_mesh_from_filename(
                str(self.output_dir / "tsdf_mesh.ply"), target_num_faces=self.target_num_faces
            )
            CONSOLE.print("Texturing mesh with Luxen")
...
class ExportPoissonMesh(Exporter):
    ...
    num_rays_per_batch: int = 32768
    """Number of rays to evaluate per batch. Decrease if you run out of memory."""
    texture_method: Literal["point_cloud", "luxen"] = "luxen"
    """Method to texture the mesh with. Either 'point_cloud' or 'luxen'."""
    px_per_uv_triangle: int = 4
    """Number of pixels per UV triangle."""
    unwrap_method: Literal["xatlas", "custom"] = "xatlas"
    """The method to use for unwrapping the mesh."""
    ...
    def main(self) -> None:
        ...
        # This will texture the mesh with Luxen and export to a mesh.obj file
        # and a material and texture file
        if self.texture_method == "luxen":
...
class ExportGaussianSplat(Exporter):
    ...
    def write_ply(
        ...
        with open(filename, "wb") as ply_file:
            luxenstudio_version = version("luxenstudio")
            # Write PLY header
            ply_file.write(b"ply\n")
            ply_file.write(b"format binary_little_endian 1.0\n")
            ply_file.write(f"comment Generated by Nerstudio {luxenstudio_version}\n".encode())
