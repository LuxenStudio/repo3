...
HEADER_LINE = "# Source luxenstudio autocompletions."


def _get_all_entry_points() -> List[str]:
    # TODO: we should filter out entrypoints that are not tyro CLIs.
    entry_points = importlib_metadata.distribution("luxenstudio").entry_points
...
def _update_conda_scripts(
    ...
    activate_path = conda_path / "etc/conda/activate.d/luxenstudio_activate.sh"
    deactivate_path = conda_path / "etc/conda/deactivate.d/luxenstudio_deactivate.sh"
    if mode == "uninstall":
...
def _get_conda_path() -> Optional[pathlib.Path]:
    ...
    the luxenstudio package is installed in one.
    """
    conda_path = None
    if "CONDA_PREFIX" in os.environ:
        # Conda is active, we will check if the Luxenstudio is installed in the conda env.
        distribution = importlib_metadata.distribution("luxenstudio")
        if str(distribution.locate_file("luxenstudio")).startswith(os.environ["CONDA_PREFIX"]):
            conda_path = pathlib.Path(os.environ["CONDA_PREFIX"])
    return conda_path
