...
"""
luxenstudio_blender.py
"""

bl_info = {
    "name": "Luxenstudio Add-On",
    "description": "Create a Luxenstudio JSON camera path from the Blender camera path \
    or import a Luxenstudio camera path as a Blender camera to composite Blender renders \
    over a Luxen background render for VFX",
    "author": "Cyrus Vachha",
    "version": (1, 0),
    "blender": (3, 0, 0),
...
class CreateJSONCameraPath(bpy.types.Operator):
    ...
    def get_camera_coordinates(self):
        ...
        luxen_mesh_mat_list = []  # list of world matrix of the Luxen mesh at each frame
        ...
        while curr_frame <= bpy.context.scene.frame_end:
            ...
            if bpy.context.scene.render.resolution_y >= bpy.context.scene.render.resolution_x:
                ...
                if self.cam_obj.data.sensor_fit == "HORIZONTAL":
                    ...
                    luxenstudio_fov = 2 * atan(tan(self.cam_obj.data.angle / 2.0) * cam_aspect_ratio)
                else:
                    # sensor fit is either vertical or auto
                    luxenstudio_fov = self.cam_obj.data.angle
            ...
            self.fov_list += [degrees(luxenstudio_fov)]
            curr_frame += bpy.context.scene.frame_step
            luxen_mesh_mat_list += [self.luxen_bg_mesh.matrix_world.copy()]

            # case when step size is 0 there is only one frame
            if bpy.context.scene.frame_step == 0:
                break

        # transform the camera world matrix based on the Luxen mesh transformation
    ...
    def construct_json_obj(self):
        ...
        if cam_type == "PERSP":
            ...
            self.report(
                {"WARNING"}, "Luxenstudio Add-on Warning: Only perspective and equirectangular cameras are supported"
            )
            cam_type = "perspective"
    ...
    def execute(self, context):
        ...
        self.luxen_bg_mesh = context.scene.Luxen
        self.file_path_json = context.scene.JSONInputFilePath

        # check input
        if self.luxen_bg_mesh is None:
            self.report(
                {"ERROR"}, "Luxenstudio add-on Error! - Please input Luxen representation (as mesh or point cloud)"
            )
            return {"FINISHED"}

        if self.file_path_json == "":
            self.report({"ERROR"}, "Luxenstudio add-on Error! - Please input a file path for the output JSON")
            return {"FINISHED"}
...
class ReadJSONinputCameraPath(bpy.types.Operator):
    """Create a camera with an animation path based on an input Luxenstudio JSON."""
    ...
    def read_camera_coodinates(self):
        ...
        for cam_keyframe in json_cam_path:
            ...
            # matrix transformation based on the luxen mesh to find relative camera positions
            self.transformed_camera_path_mat += [self.luxen_bg_mesh.matrix_world.copy() @ orig_cam_mat]
    ...
    def generate_camera(self):
        ...
        camera_data = bpy.data.cameras.new(name="LuxenstudioCamera")
        camera_data = bpy.data.cameras.new(name="LuxenstudioCamera")
        luxenstudio_camera_object = bpy.data.objects.new("LuxenstudioCamera", camera_data)
        bpy.context.scene.collection.objects.link(luxenstudio_camera_object)

        curr_frame = 0
        while curr_frame < len(json_cam_path):
            actual_frame = curr_frame + 1
            # animate camera transform
            luxenstudio_camera_object.matrix_world = self.transformed_camera_path_mat[curr_frame]
            luxenstudio_camera_object.keyframe_insert("location", frame=actual_frame)
            luxenstudio_camera_object.keyframe_insert("rotation_euler", frame=actual_frame)

            # set scale to 1,1,1 (scale is not keyframed)
            luxenstudio_camera_object.scale = (1, 1, 1)

            # animate fov
            luxenstudio_camera_object.data.sensor_fit = "VERTICAL"
            luxenstudio_camera_object.data.lens_unit = "FOV"
            luxenstudio_camera_object.data.angle = radians(self.fov_list[curr_frame])

            # set keyframe for focal length
            luxenstudio_camera_object.data.keyframe_insert(data_path="lens", frame=actual_frame)
        ...
        if input_cam_type == "perspective":
            luxenstudio_camera_object.data.type = "PERSP"
        if input_cam_type == "equirectangular":
            luxenstudio_camera_object.data.type = "PANO"
            bpy.context.scene.render.engine = "CYCLES"
            luxenstudio_camera_object.data.cycles.panorama_type = "EQUIRECTANGULAR"
        if input_cam_type == "fisheye":
            luxenstudio_camera_object.data.type = "PERSP"
            self.report({"WARNING"}, "Luxenstudio Add-on Warning: Fisheye cameras are not supported")
    ...
    def execute(self, context):
        ...
        if file_path_ns_json == "":
            self.report({"ERROR"}, "Luxenstudio add-on Error! - Please input a Luxenstudio JSON camera path")
            return {"FINISHED"}
...
class LuxenstudioMainPanel(bpy.types.Panel):  # pylint: disable=too-few-public-methods
    ...
    def draw(self, context):
        ...
        # Luxen representation object input box
        self.layout.label(text="Luxen Representation (mesh or point cloud)")
        self.layout.prop_search(context.scene, "Luxen", context.scene, "objects")
...
class LuxenstudioBgPanel(bpy.types.Panel):  # pylint: disable=too-few-public-methods
    ...
    bl_label = "Luxenstudio Path Generator"
    bl_parent_id = "NERFSTUDIO_PT_LuxenstudioMainPanel"
    bl_space_type = "PROPERTIES"
    bl_region_type = "WINDOW"
    bl_context = "render"
    ...
    def draw(self, context):
        ...
        self.layout.label(text="Camera path for Luxenstudio")
...
class LuxenstudioInputPanel(bpy.types.Panel):  # pylint: disable=too-few-public-methods
    ...
    bl_label = "Luxenstudio Camera Generator"
    bl_parent_id = "NERFSTUDIO_PT_LuxenstudioMainPanel"
    bl_space_type = "PROPERTIES"
    bl_region_type = "WINDOW"
    bl_context = "render"
    ...
    def draw(self, context):
        ...
        col = self.layout.column()
        self.layout.label(text="Create Blender Camera From Luxenstudio JSON")
        col = self.layout.column()
...
CLASSES = [
    LuxenstudioMainPanel,
    LuxenstudioBgPanel,
    LuxenstudioInputPanel,
...
INPUT_PROPERTIES_NS_CAMERA = [
    (
        ...
        bpy.props.StringProperty(
            name="JSON Luxenstudio File",
            default="",
            description="Path for JSON from Luxenstudio editor",
            subtype="FILE_PATH",
        ),
    )
]

OBJ_PROPERTIES = ["Luxen", "RenderCamera"]
...
def register():
    ...
    bpy.types.Scene.Luxen = bpy.props.PointerProperty(type=bpy.types.Object)
