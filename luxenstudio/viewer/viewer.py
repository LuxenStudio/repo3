...
from luxenstudio.viewer.metrics_panel import populate_metrics_tab
...
class Viewer:
    ...
    def __init__(
        ...
        with control_tab:
            from luxenstudio.viewer_legacy.server.viewer_elements import ViewerElement as LegacyViewerElement

            if len(parse_object(pipeline, LegacyViewerElement, "Custom Elements")) > 0:
                from luxenstudio.utils.rich_utils import CONSOLE

                CONSOLE.print(
                    "Legacy ViewerElements detected in model, please import luxenstudio.viewer.viewer_elements instead",
                    style="bold yellow",
                )
