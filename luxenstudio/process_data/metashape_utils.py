...
"""Helper utils for processing metashape data into the luxenstudio format."""
...
def metashape_to_json(
    ...
    """Convert Metashape data into a luxenstudio dataset.
    ...
    if sensor_type.count(sensor_type[0]) != len(sensor_type):
        raise ValueError(
            ...
            "luxenstudio does support per-frame camera intrinsics, but at this current time the utility does not account for this."
        )
    data = {}
