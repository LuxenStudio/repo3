...
from luxenstudio.utils.math import safe_normalize
from luxenstudio.utils.spherical_harmonics import components_from_spherical_harmonics
...
class RGBRenderer(nn.Module):
    ...
    def combine_rgb(
        ...
        if ray_indices is not None and num_rays is not None:
            ...
            comp_rgb = luxenacc.accumulate_along_rays(
                weights[..., 0], values=rgb, ray_indices=ray_indices, n_rays=num_rays
            )
            accumulated_weight = luxenacc.accumulate_along_rays(
...
class AccumulationRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            accumulation = luxenacc.accumulate_along_rays(
                weights[..., 0], values=None, ray_indices=ray_indices, n_rays=num_rays
            )
        else:
...
class DepthRenderer(nn.Module):
    ...
    def forward(
        ...
        if self.method == "expected":
            ...
            if ray_indices is not None and num_rays is not None:
                # Necessary for packed samples from volumetric ray sampler
                depth = luxenacc.accumulate_along_rays(
...
class SemanticRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            return luxenacc.accumulate_along_rays(
