...
class RGBRenderer(nn.Module):
    ...
    def combine_rgb(
        ...
        if ray_indices is not None and num_rays is not None:
            ...
            comp_rgb = luxenacc.accumulate_along_rays(weights.squeeze(-1), rgb, ray_indices, num_rays)
            accumulated_weight = luxenacc.accumulate_along_rays(weights.squeeze(-1), None, ray_indices, num_rays)
        else:
...
class AccumulationRenderer(nn.Module):
    ...
    def forward(
        ...
        if ray_indices is not None and num_rays is not None:
            # Necessary for packed samples from volumetric ray sampler
            accumulation = luxenacc.accumulate_along_rays(weights.squeeze(-1), None, ray_indices, num_rays)
        else:
...
class DepthRenderer(nn.Module):
    ...
    def forward(
        ...
        if self.method == "expected":
            ...
            if ray_indices is not None and num_rays is not None:
                # Necessary for packed samples from volumetric ray sampler
                depth = luxenacc.accumulate_along_rays(weights.squeeze(-1), steps, ray_indices, num_rays)
                accumulation = luxenacc.accumulate_along_rays(weights.squeeze(-1), None, ray_indices, num_rays)
