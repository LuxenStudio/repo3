...
import luxenacc
import torch
from luxenacc import OccupancyGrid
from torch import nn
from torchtyping import TensorType

from luxenstudio.cameras.rays import Frustums, RayBundle, RaySamples
...
class VolumetricSampler(Sampler):
    ...
    def forward(
        ...
        ray_indices, starts, ends = luxenacc.ray_marching(
            rays_o=rays_o,
            rays_d=rays_d,
            t_min=t_min,
            t_max=t_max,
...
class ProposalNetworkSampler(Sampler):
    ...
    Args:
        ...
        num_luxen_samples_per_ray: Number of samples to generate per ray for the NERF model.
    ...
    def generate_ray_samples(
        ...
        for i_level in range(n + 1):
            is_prop = i_level < n
            num_samples = self.num_proposal_samples_per_ray[i_level] if is_prop else self.num_luxen_samples_per_ray
            if i_level == 0:
