...
"""
Implementation of LuxenPlayer (https://arxiv.org/abs/2210.15947) with InstantNGP backbone.
"""
...
from luxenstudio.fields.luxenplayer_ngp_field import LuxenplayerNGPField
...
from luxenstudio.models.instant_ngp import InstantNGPModelConfig, NGPModel


@dataclass
class LuxenplayerNGPModelConfig(InstantNGPModelConfig):
    """LuxenPlayer Model Config with InstantNGP backbone.
    ...
    3. If the input cameras are of limited numbers, try reducing `num_levels`. `num_levels` is for
        multi-resolution volume sampling, and has a similar behavior to the freq in Luxen. With a small
        `num_levels`, a blurred rendering will be generated, but it is unlikely to overfit the training views.
    """

    _target: Type = field(default_factory=lambda: LuxenplayerNGPModel)
    temporal_dim: int = 64
...
class LuxenplayerNGPModel(NGPModel):
    """LuxenPlayer Model with InstantNGP backbone.

    Args:
        config: LuxenPlayer NGP configuration to instantiate model
    """

    config: LuxenplayerNGPModelConfig
    field: LuxenplayerNGPField
    ...
    def populate_modules(self):
        ...
        # Occupancy Grid
        self.occupancy_grid = luxenacc.OccupancyGrid(
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        weights = luxenacc.render_weight_from_density(
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        if "depth_image" in batch.keys() and self.config.depth_weight > 0:
            ...
            # (i.e., `distortion_loss` in luxenstudio) is used to make surface tight.
            ...
            # (Try luxenplayer with luxenacto backbone for distortion loss.)
