...
"""
Luxenstudio's Text to 3D model.
"""
...
@dataclass
class GeluxenactoModelConfig(ModelConfig):
    """Geluxenacto model config"""

    _target: Type = field(default_factory=lambda: GeluxenactoModel)
...
class GeluxenactoModel(Model):
    """Geluxenacto Model

    Args:
        config: Geluxenacto configuration to instantiate model
    """

    config: GeluxenactoModelConfig
    ...
    def populate_modules(self):
        ...
        # setting up fields
        self.field = GeluxenactoField(self.scene_box.aabb, max_res=self.config.max_res)
