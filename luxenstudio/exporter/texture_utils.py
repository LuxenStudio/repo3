...
from luxenstudio.utils.rich_utils import CONSOLE, get_progress
...
def unwrap_mesh_per_uv_triangle(
    ...
    interpolation to get the origins and directions, per pixel, that are needed to render the Luxen with.
    ...
    # which can be used to interpolate the XYZ and normal values to then query with Luxen
...
def export_textured_mesh(
    ...
    CONSOLE.print("Creating texture image by rendering with Luxen...")
    with torch.no_grad():
    ...
    # create the .obj file
    lines_obj = ["# Generated with luxenstudio", "mtllib material_0.mtl", "usemtl material_0"]
