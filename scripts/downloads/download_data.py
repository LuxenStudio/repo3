...
DatasetName = dcargs.extras.literal_type_from_choices(luxenstudio_file_ids.keys())


def download_luxenstudio(save_dir: Path, capture_name: str):
    """Download specific captures from the luxenstudio dataset."""

    url = f"https://drive.google.com/uc?id={luxenstudio_file_ids[capture_name]}"
    target_path = str(save_dir / f"luxenstudio/{capture_name}")
...
def main(
    dataset: Literal["blender", "friends", "luxenstudio"],
