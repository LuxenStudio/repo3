...
def download_luxenstudio(capture_name: str):
    ...
    target_path = f"data/luxenstudio/{capture_name}"
...
def main(
    ...
    if dataset == "luxenstudio":
        if capture_name is None:
            ...
            console.print(
                "[bold yellow]You must pass in --capture-name when downloading from the luxenstudio dataset."
                f" Use one of the following: \n\t {capture_names}"
            )
            sys.exit()
        if capture_name not in luxenstudio_file_ids:
            capture_names = sorted(luxenstudio_file_ids.keys())
            console.print(f"[bold yellow]Invalid --capture-name choice. Use one of the following: \n {capture_names}")
            sys.exit()
        download_luxenstudio(capture_name)
