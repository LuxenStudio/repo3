...
def download_friends():
    ...
    data_filename_to_file_id = [
        ...
        ("./data/sparse_reconstruction_and_luxen_data.zip", "1RmwDUAp1T4RkwZg1S_7L9JAViWtBZfAG"),
    ]

    disk_data_filename_to_file_id = [  # pylint: disable=unused-variable
        ("./data/sparse_reconstruction_and_luxen_data/ELR-apartment-disk.zip", "1YcTwMv5PP0uqXYdN-Lp8k0WnXUdHBXrY"),
        ("./data/sparse_reconstruction_and_luxen_data/Frasier-apartment-disk.zip", "1vPqplDv5rrFu5LrxTHbET0FiePTGi8zE"),
        (
            "./data/sparse_reconstruction_and_luxen_data/Friends-monica_apartment-disk.zip",
            "1yNpU4M44gIuWEaC5tSAOKy-A6KX8wTm8",
        ),
        (
            "./data/sparse_reconstruction_and_luxen_data/HIMYM-red_apartment-disk.zip",
            "1m76i46YYlpDKvlx-Jn7qDDAvkOVVVmqF",
        ),
        (
            "./data/sparse_reconstruction_and_luxen_data/Seinfeld-jerry_living_room-disk.zip",
            "1YFoV3Gd9asKsRarZvzRTYtri1755rBPx",
        ),
        ("./data/sparse_reconstruction_and_luxen_data/TAAHM-kitchen-disk.zip", "1QpF8kVGfgkqm_cDY5sY_E82BT8VOWL38"),
        (
            "./data/sparse_reconstruction_and_luxen_data/TBBT-big_living_room-disk.zip",
...
def download_blender():
    ...
    unzip_path = Path("data/luxen_synthetic")
    final_path = Path("data/blender")
    unzip_path.rename(final_path)
    download_path.unlink(missing_ok=True)


def download_luxenactory(dataset_name: str):
    """Download a zipped luxenactory dataset"""
    url = f"https://drive.google.com/uc?id={luxenactory_file_ids[dataset_name]}"
...
def main(
    dataset: Literal["blender", "friends", "luxenactory"],
    ...
    if dataset == "friends":
        download_friends()
    if dataset == "luxenactory":
        if dataset_name is None or dataset_name not in luxenactory_file_ids:
            raise ValueError(
                f"must pass in dataset_name when downloading luxenactory data, options: {luxenactory_file_ids.keys()}"
            )
        download_luxenactory(dataset_name)
