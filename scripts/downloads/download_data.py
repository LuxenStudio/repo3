...
DatasetName = tyro.extras.literal_type_from_choices(luxenstudio_file_ids.keys())
...
def download_luxenstudio(save_dir: Path, capture_name: str):
    """Download specific captures for the luxenstudio dataset."""
    download_capture_name(save_dir, "luxenstudio", capture_name, capture_name_to_file_id=luxenstudio_file_ids)
...
def download_dluxen(save_dir: Path):
...
def main(
    dataset: Literal["blender", "friends", "luxenstudio", "record3d", "dluxen"],
    ...
    Args:
        ...
        capture_name: The capture name to download (if downloading from luxenstudio dataset).
    ...
    if dataset == "luxenstudio":
        if capture_name is None:
            capture_names = sorted(luxenstudio_file_ids.keys())
            CONSOLE.rule("[bold red]Error", style="bold red")
            CONSOLE.print("[bold yellow]You must pass in --capture-name when downloading from the luxenstudio dataset.")
            CONSOLE.print("Use one of the following:")
            CONSOLE.print(f"\t {capture_names}")
            sys.exit()
        download_luxenstudio(save_dir, capture_name)
    if dataset == "record3d":
        download_record3d(save_dir, "bear")
    if dataset == "dluxen":
        if capture_name is not None:
            CONSOLE.print("Capture name is ignored when downloading from the dluxen dataset.")
        download_dluxen(save_dir)
