...
Download the original Luxen Blender dataset. We support the major datasets and allow users to create their own dataset, described in detail [here](https://docs.luxen.studio/en/latest/quickstart/custom_dataset.html).
...
If you have custom data in the form of a video or folder of images, we've provided some [COLMAP](https://colmap.github.io/) and [FFmpeg](https://ffmpeg.org/download.html) scripts to help you process your data so it is compatible with luxenstudio.
...
ns-train luxenacto --vis viewer

# with a specified websocket port
ns-train luxenacto --vis viewer --viewer.websocket-port=7008
...
```bash
# assuming previously ran `ns-train luxenacto`
ns-render --load-config=outputs/data-luxenstudio-poster/luxenacto/{TIMESTAMP}/config.yml --traj=spiral --output-path=renders/output.mp4
```
